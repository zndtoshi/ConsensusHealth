...
test("distant galaxy centers stay inset so labels fit 1280 and mobile widths", () => {
  ...
  for (const id of ids) {
    ...
    assert.ok(pos.x >= 7 && pos.x <= 93, `${id} x=${pos.x}`);
    assert.ok(pos.scale <= 0.72, `${id} scale=${pos.scale}`);
    ...
    const mobileLeft = Math.max(48, Math.min(390 - 48, (pos.x / 100) * 390));
...
test("unknown BIP fallback does not alter canonical slots", () => {
  ...
  assert.ok(unknown.x >= 7 && unknown.x <= 93);
});
